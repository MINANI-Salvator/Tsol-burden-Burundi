### TAENIA SOLIUM IN BURUNDI - SENSITIVITY ANALYSES
...
## load cost script
source("Tsolium burundi cost-30-9.R")
...
sa_src(cost_ncc,
       cbind(e_prev, prop_ncc,
             p_hosp, n_visit_med, p_treatment,
             working_days, loss_workingtime, monthly_salary))

sa_pcc(cost_ncc,
       cbind(e_prev, prop_ncc,
             p_hosp, n_visit_med, price_med, p_treatment,
             working_days, loss_workingtime, monthly_salary))
...
c("Monthly salary", "Prevalence of E", "Proportion of NCC-associated E", 
  "Cost of a pig", "Prevalence of PC", "Working days", "Working time lost", 
  "Proportion of hospitalizations", "Cost for health care provider",
...
## load cost script
source("Tsolium burundi daly-30-9.R")
